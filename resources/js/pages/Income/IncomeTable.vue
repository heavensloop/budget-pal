...
const emit = defineEmits<{
    ...
    archive: [item: IncomeItem];
    restore: [item: IncomeItem];
    destroy: [item: IncomeItem];
}>();
...
// Income without a schedule is assumed to repeat monthly by default; we
...
function nextPaymentLabel(item: IncomeItem): string {
...
function toggleExpanded(item: IncomeItem) {
...
function onArchiveOrRestore(item: IncomeItem) {
...
<template>
    ...
    <div v-else>
        ...
        <!-- Cards: below md -->
        <div class="flex flex-col gap-2 md:hidden" data-testid="income-cards">
            <div
                v-for="item in items"
                :key="item.id"
